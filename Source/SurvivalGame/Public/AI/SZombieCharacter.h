...
class SURVIVALGAME_API ASZombieCharacter : public ASBaseCharacter
{
	...
	// Notify the game mode when this zombie dies
	void NotifyZombieDeath();

protected:
	// The type of ammo that can damage this zombie
	UPROPERTY(BlueprintReadWrite, Category = "Zombie|Vulnerability")
	EAmmoType VulnerableAmmoType;
...
public:
	...
	// Getter for the vulnerable ammo type
	UFUNCTION(BlueprintCallable, Category = "Zombie|Vulnerability")
	EAmmoType GetVulnerableAmmoType() const;

	// Setter for the vulnerable ammo type (optional)
	void SetVulnerableAmmoType(EAmmoType NewType);
