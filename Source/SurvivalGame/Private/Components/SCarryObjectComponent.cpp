...
void USCarryObjectComponent::Pickup()
{
	...
	// Directly get the actor in view and perform pickup
	AActor* FocusActor = GetActorInView();
	OnPickup(FocusActor); // Call the simplified OnPickup
}
...
void USCarryObjectComponent::Drop()
{
	...
	if (CarriedActor)
	{
		// Find the static mesh to re-enable physics simulation
		...
		// Detach the actor from the component
...
void USCarryObjectComponent::Throw()
{
	...
	if (MeshComp)
	{
		...
		{
			/* Re-map uint8 to 360 degrees */
			const float PawnViewPitch = (OwningPawn->GetControlRotation().Pitch);
...
void USCarryObjectComponent::Rotate(float DirectionYaw, float DirectionRoll)
{
	...
	if (CarriedActor)
	{
		...
		FVector RootOrigin = CarriedActor->GetRootComponent()->Bounds.Origin;
...
void USCarryObjectComponent::OnPickup(AActor* FocusActor)
{
