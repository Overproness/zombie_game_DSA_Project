	...
	: Super(ObjectInitializer)
{
	...
	//// Load and assign the skeletal mesh
	// static ConstructorHelpers::FObjectFinder<USkeletalMesh> ZombieMesh(TEXT("SkeletalMesh'/Game/ZombieCharacters/Zombie1/Girlscout_T_Masuyama.Girlscout_T_Masuyama'"));
	// if (ZombieMesh.Succeeded())
	//{
	//	//UE_LOG(log, _CRT_WARNING_MESSAGE, "Mesh Assigning succeeded. ");
	//	GetMesh()->SetSkeletalMesh(ZombieMesh.Object);
	// }

	//// Load and assign the physics asset
	// static ConstructorHelpers::FObjectFinder<UPhysicsAsset> ZombiePhysics(TEXT("/Game/Path/To/YourPhysicsAsset.YourPhysicsAsset"));
	// if (ZombiePhysics.Succeeded())
	//{
	//	GetMesh()->SetPhysicsAsset(ZombiePhysics.Object);
	// }

	//// Set additional mesh properties
	...
	// Set default vulnerable ammo type
	int32 RandomIndex = FMath::RandRange(0, 4); // Generates 0, 1, or 2
	EAmmoType RandomAmmoType = static_cast<EAmmoType>(RandomIndex);
	SetVulnerableAmmoType(RandomAmmoType);
}
...
EAmmoType ASZombieCharacter::GetVulnerableAmmoType() const
{
	return VulnerableAmmoType;
}

void ASZombieCharacter::SetVulnerableAmmoType(EAmmoType NewType)
{
	VulnerableAmmoType = NewType;
}

bool ASZombieCharacter::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
	if (!Super::Die(KillingDamage, DamageEvent, Killer, DamageCauser))
	{
		return false;
	}

	// Notify game mode about the zombie's death
	ASGameMode* GameMode = Cast<ASGameMode>(GetWorld()->GetAuthGameMode());
	if (GameMode)
	{
		GameMode->OnZombieDefeated();
	}
