...
#include "Kismet/KismetMathLibrary.h" // For random integer generation

// For adding mesh programmatically
...
{
	// Maintain existing game state updates (if any)
	if (IsMatchInProgress())
	{
		ASGameState* MyGameState = Cast<ASGameState>(GameState);
		if (MyGameState)
		{
			// Remove dynamic time update logic
			//MyGameState->SetNightState(); // Optional: Update night state if needed for other mechanics
		}
	}
}

void ASGameMode::StartLevel(int LevelNumber)
{
	if (!TimeOfDayManager)
	{
		TimeOfDayManager = GetWorld()->SpawnActor<ASTimeOfDayManager>();
	}

	// Set static time in the game state
	ASGameState* MyGameState = Cast<ASGameState>(GameState);
	if (MyGameState)
	{
		MyGameState->SetLevelTime(LevelNumber);
	}

	// Start the wave for this level
	SpawnZombieWave(LevelNumber);
}


FVector GenerateRandomSpawnLocation()
{
	FVector Origin(0.0f, 0.0f, 200.0f); // Adjust base location
	FVector Range(1000.0f, 1000.0f, 0.0f); // Adjust range
	return Origin + FMath::RandPointInBox(FBox(-Range, Range));
}

void ASGameMode::SpawnZombieWave(int LevelNumber)
{
	int NumZombies = LevelNumber * 10; // Example: 10 zombies per level
	ZombiesRemaining = NumZombies;

	for (int i = 0; i < NumZombies; ++i)
	{
		// Call SpawnNewBot to handle the spawning logic
		SpawnNewBot();
	}

	UE_LOG(LogTemp, Log, TEXT("Spawned %d zombies for Level %d"), NumZombies, LevelNumber);
}
...
{
	if (!BotPawnClass) // Validate BotPawnClass
	{
		UE_LOG(LogGame, Error, TEXT("BotPawnClass is not set. Cannot spawn bots!"));
		return;
	}

	FTransform SpawnTransform;
	if (!FindBotSpawnTransform(SpawnTransform)) // Validate spawn location
	{
	...
	// Spawn the bot
	APawn* SpawnedBot = GetWorld()->SpawnActor<APawn>(BotPawnClass, SpawnTransform);
	if (SpawnedBot)
	{
		ASZombieCharacter* SpawnedZombie = Cast<ASZombieCharacter>(SpawnedBot);
		if (SpawnedZombie)
		{
			//// Generate a random ammo type
			//int32 RandomIndex = FMath::RandRange(0, 2); // Generates 0, 1, or 2
			//EAmmoType RandomAmmoType = static_cast<EAmmoType>(RandomIndex);

			//// Assign the vulnerability
			//SpawnedZombie->SetVulnerableAmmoType(RandomAmmoType);

			//// Log the spawned zombie's ammo vulnerability
			//FString AmmoTypeName;
			//switch (RandomAmmoType)
			//{
			//case EAmmoType::NineMM:   AmmoTypeName = TEXT("NineMM"); break;
			//case EAmmoType::Shotgun:  AmmoTypeName = TEXT("Shotgun"); break;
			//case EAmmoType::Rifle:    AmmoTypeName = TEXT("Rifle"); break;
			//default:                  AmmoTypeName = TEXT("Unknown"); break;
			//}

			//UE_LOG(LogGame, Log, TEXT("Spawned zombie with Ammo Vulnerability: %s"), *AmmoTypeName);
		}
		else
		{
			UE_LOG(LogGame, Warning, TEXT("Spawned bot is not a zombie character."));
		}
	}
	else
	{
		UE_LOG(LogGame, Error, TEXT("Failed to spawn bot."));
	}
}
...
{
	UE_LOG(LogTemp, Warning, TEXT("SpawnBotHandler called"));
	...
	if (MyGameState)
	{
		...
		if (MyGameState->GetIsNight())
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Max pawns in zone reached: %d"), PawnsInWorld);
			}
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("It's not night. Zombies won't spawn."));
		}
	}
}
...
void ASGameMode::OnZombieDefeated()
{
	ZombiesRemaining--;

	UE_LOG(LogTemp, Log, TEXT("Zombie defeated. Remaining: %d"), ZombiesRemaining);
...
void ASGameMode::OnWaveCompleted()
{
	UE_LOG(LogTemp, Log, TEXT("Wave completed for Level %d"), CurrentLevel);

	if (CurrentLevel < 3) // Example: 3 levels in total
	{
		CurrentLevel++;
	...
	{
		UE_LOG(LogTemp, Log, TEXT("Game Completed!"));
		EndGame(); // Trigger custom endgame logic
	}
}

void ASGameMode::EndGame()
{
	UE_LOG(LogTemp, Log, TEXT("Game Over. Displaying End Screen..."));
	// Implement endgame logic, such as showing UI or stopping gameplay
}
