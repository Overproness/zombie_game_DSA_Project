	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<USCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
	...
	HealthQueue = CreateDefaultSubobject<UArrayQueue>(TEXT("HealthQueue"));
...
{
	Super::BeginPlay();
	// Initialize health consumption timer
	GetWorldTimerManager().SetTimer(HealthConsumeTimer, this, &ASBaseCharacter::ConsumeHealthItem, 2.0f, true, -1.0f); // 2-second delay between consumption
}

void ASBaseCharacter::AddHealthItemToQueue()
{
	// Enqueue health item only if the queue is not full
	// Enqueue health item only if the queue is not full
	if (!HealthQueue->IsFull())
	{
		HealthQueue->Enqueue(25); // Assuming each food pallet restores 25 health
		UE_LOG(LogTemp, Log, TEXT("Health item added to queue. Current queue size: %d"), HealthQueue->GetSize());
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("Health queue is full. Item not added."));
	}
}

void ASBaseCharacter::ConsumeHealthItem()
{
	if (Health >= GetMaxHealth() || HealthQueue->IsEmpty())
	{
		// Stop consuming if health is full or the queue is empty
		return;
	}

	int32 HealthToRestore = HealthQueue->Dequeue();
	if (HealthToRestore)
	{
		Health = FMath::Min(Health + HealthToRestore, GetMaxHealth());
		UE_LOG(LogTemp, Log, TEXT("Consumed health item. Current health: %f, Queue size: %d"), Health, HealthQueue->GetSize());
	}
}

void ASBaseCharacter::StartHealthConsumption()
{
	if (GetWorldTimerManager().IsTimerActive(HealthConsumeTimer))
	{
		GetWorldTimerManager().ClearTimer(HealthConsumeTimer);
	}
	GetWorldTimerManager().SetTimer(HealthConsumeTimer, this, &ASBaseCharacter::ConsumeHealthItem, 2.0f, true);
}
...
void ASBaseCharacter::OnDeath(float KillingDamage, FDamageEvent const& DamageEvent, APawn* PawnInstigator, AActor* DamageCauser)
{
    ...
    // Spawn the Ammo1BP class on death
    if (Ammo1BP)
    {
        FVector SpawnLocation = GetActorLocation(); // Spawn at the character's location
        ...
        GetWorld()->SpawnActor<AActor>(Ammo1BP, SpawnLocation, SpawnRotation, SpawnParams);
    }
